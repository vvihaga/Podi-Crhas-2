...
const { state, saveCreds } = await useMultiFileAuthState("ranssesi")
...
const phoneNumber = await question(chalk.yellowBright('[•] Masukan nomor bot dengan awalan dengan 62:\n'));
console.log(chalk.magentaBright('[!] masukan key:'));
const codee = await question(chalk.cyanBright('[•] code key: '));
        if (codee !== 'goku') {
        console.log(chalk.redBright('[✖] key nya salah tolol '));
            return;}
        console.log(chalk.greenBright('[✔] key salah goblok pi boong'));
console.log(chalk.magentaBright('[!] masukan key untuk mendapatkan pairing code:'));
const code = await question(chalk.cyanBright('[•] code key: '));
        if (code !== 'petra-rans-kahfi') {
        console.log(chalk.redBright('[✖] key nya salah tolol '));
            return;}
        console.log(chalk.greenBright('[✔] key salah goblok pi boong'));
        let pairingCode = await client.requestPairingCode(phoneNumber);
        pairingCode = pairingCode?.match(/.{1,4}/g)?.join("-") || pairingCode;
        console.log(chalk.magentaBright(`YouPairing: ${pairingCode}`))
}
...
require("./goku")(client, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
console.log('whatsapp connection successfull\n ' + JSON.stringify(client.user.id, null, 2));
}
});
